...
- [~] **Onglet Qualité — #5** ← `SUIVI DES ANALYSES` (gsheet `1lE9te1…Jzi-c`). **PROFILÉ 30/06** → `docs/profil_suivi_analyses.md` (6 blocs ; cœur = A suivi + D archive ~336 lignes ; E facturation ~107 ; B/F/G référentiels). `CA` = `ref_rapport` (jointure #1/#2). 🔴 **Décalage de grain** : #5 = plusieurs analyses/(PO,article) (stades, CA) vs `achat.qualite` UNIQUE(po,article) → décision : table `qualite_suivi` (grain échantillon, reco) + `qualite_facturation` (bloc E). Puis `transform_suivi_analyses`.
