...
- [~] **Onglet Artwork** ← `LIS-CON-28-0 Suivi des artworks-import` (gsheet `1FTr2nl…J4Jrc`, Drive *Design et Achat*). **PROFILÉ 30/06** → `docs/profil_artworks.md` (2 tableaux empilés ~473 lignes, 8 gotchas). 🔴 **Décalage de grain** : gsheet par ARTICLE (clé Référence, pas de PO) vs `achat.artwork` par (po_number, code_article) → décision requise (table `artwork_statut` + merge, comme pattern A). Clé = Référence. Filtrer #N/A / « PAS DE REF ».
