...
| 7 | Drive TB/GDD (Inspection + Results of analysis) | `achat.qualite_doc` (index) + `achat.qualite_analyse` (mesures, non fait) | Lien FAIL→rapport (index) ; chrome/dureté (mesures, bloqué) | Moyen (index) / Élevé (mesures, parsing PDF) | 🟡 **PILOTE 02/07** — index : 8 fichiers / 2 PO chargés (`load_qualite_doc_drive.py`). Mesures SPECTRO (chrome/dureté) **non faites** : texte Drive aplati, pdfplumber pas validable en session (sandbox HS). À reprendre avec pdfplumber sur PDF local (poste ou serveur prod) avant de peupler `qualite_analyse` — ne jamais insérer une valeur non fiable. Passage à l'échelle (~48 PO TB + GDD) : étendre `RECORDS` ou crawler le serveur `ANALYSES ET INSPECTIONS` directement en prod. |
