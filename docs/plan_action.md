...
- [x] **Alertes changement ETA/livraison — implémenté le 23/07 & 27/07** (`docs/20260722_FUSEAU_Spec_SuiviDatesETA_v1.md`) :
  - DDL : réutilise `achat.transport_evenement` + vue `achat.v_ot_transport_suivi` (nb changements cumulés + couleur).
  - Déblocage BDD (`3a3ba42`) : contournement d'ID subquery appliqué sur les insertions d'événements, 100% débloqué sur Azure.
  - Ingestion Mails Transitaires (`7baf80e`) : module `parse_email_eta.py` + `load_email_eta.py` pour extraire automatiquement les réestimations d'ETA et motifs de retard depuis le corps des emails QUALITAIRSEA, Bolloré, Sealogis, etc.
- [x] **Fiche Achat Phase B — fait le 27/07** (`ec5ed3c`) : export PDF (`@media print` A4) + export Excel `.xlsx` conforme cellule par cellule au gabarit officiel **FOR-ACH-03-12** via `POST /api/fiche-achat/export-excel`.
- [x] **Hébergement Global & Pérennisation — en cours (27/07)** : Samuel (Infra/Nubo) prépare le Windows Server dédié pour l'hébergement global de FUSEAU (API uvicorn, frontend, et tâche planifiée Task Scheduler nocturne via `run_etl_scheduled.ps1`).
...
**Mise à jour 27/07** : Déblocage BDD Azure OK, Fiche Achat Phase B OK (PDF/Excel), Ingestion Mails ETA Transitaires OK. Samuel (Infra) prépare le Windows Server dédié pour héberger FUSEAU au global.
