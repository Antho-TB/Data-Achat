...
- [x] **WANXIN : retard maxi = 444 jours -- VERIFIE (23/07), ce n'est PAS une saisie erronee.** PO 145862 (46 lignes) : etd_confirme = 2024-09-05 pour la quasi-totalite des lignes, alors que les articles ont reellement expedie en lots etales jusqu'en 2025-11-23 (etd_reel) puis livres jusqu'en 2026-01-14 -- dates toutes coherentes et croissantes, aucun artefact de parsing. Confirme en base : chaque PO WANXIN ne porte qu'1 seule valeur etd_confirme partagee par toutes ses lignes (fixee a la confirmation de commande, jamais reactualisee par article/lot). Le retard n'est donc pas un retard fournisseur mais l'ecart mecanique entre la date de commande initiale et l'expedition reelle du dernier lot d'une commande etalee dans le temps. Rejoint directement la question [DEC] deja ouverte sur la semantique d'etd_confirme (date de depart ferme vs date de commande initiale) -- ce n'est pas une anomalie isolee a corriger, c'est la manifestation la plus extreme du meme probleme de fond.

### 🟡 Décisions métier requises avant dev (deadline Andréa 31/07)
- [x] [DEC] **Statut "Livrée"** : recherche DWH Sylob faite (23/07). **Aucune table Sylob accessible ne porte une date de réception réelle jour-par-jour, jointe proprement à PO+article** : `receptions2`/`receptions_detaillees2` (et `receptions4`/`_detaillees4`) sont bien joignables sur PO (`commande_numero_de_la_commande`) + article (`article_code_article`) via `reception_code`, mais leur seul champ date (`date_reception2`) est un **entier (année seule)**, pas une date. Le champ qui porte vraiment `date_reception_reelle` (type `date`) existe dans `bi_reporting.fact_achats_consolides`, mais (a) **accès refusé** au login de service actuel (`dtpf_sylob_anthony_bezille_prod`, permission denied sur le schéma `bi_reporting`), et (b) sa grain n'expose ni PO ni code article (juste id/société/fournisseur/quantité) — inutilisable en l'état pour un rapprochement par ligne. Piste alternative explorée : `mouvements_de_stock22.car_artstk_lot_date_d_entree` (date d'entrée en stock par lot/article) existe mais n'a **aucune colonne PO** pour rattacher à une commande. → **Bloqué sans action IT** : soit obtenir l'accès au schéma `bi_reporting` (à demander à qui gère les logins Sylob) pour vérifier si la table peut être enrichie/jointe côté source, soit accepter un proxy moins précis (ex. dernière `lignecommande_reception_prevue_le` comme date indicative, en le nommant clairement "prévue" et non "réelle").
- [ ] [DEC][M] **Raison du retard** : à extraire du corps des mails (parsing), pas de saisie structurée existante — rejoint le chantier "non-conformité par mail" déjà identifié. *(non traité le 23/07)*
- [x] [DEC] **Doublons fournisseurs — tranché (23/07)** : confirmé de vrais doublons à corriger via l'ID Sylob `frn_code`, pas le nom texte — **implémenté** (`13ad399`, `enrich_ca.py`) : regroupement par composantes connexes nom↔frn_code, avec garde-fou anti-faux-positif (un lien nom↔code ambigu n'est retenu qu'avec ≥ 2 PO de corroboration — un seul PO bruité aurait sinon fusionné à tort GUANGWEI/DIAMOND TRACK avec HONGXING, 2 fournisseurs distincts). Résultat : GUANGWEI/DIAMOND TRACK et SMART IRON/JIT GLOBAL fusionnés comme demandé. **AOYAM/HIAMEA confirmé également par Antho (23/07)** — même frn_code 00001234 (2 et 3 PO), 3ᵉ groupe fusionné au même titre que les 2 premiers.
- [x] [DEC] **Qualité — code article absent des rapports d'inspection** : confirmé (23/07) — **décision métier Achat/Qualité, hors scope dev FUSEAU**. Le service Qualité doit nommer ses PDF avec le code article dedans ; FUSEAU se contentera de parser ce nommage une fois la convention actée avec eux (cf. chantier ⬜ ci-dessous, reste bloqué sur ce point process).
- [x] [DEC] **Promo/Opé — filtre trop large** : tranché et **implémenté (23/07)** (`f384854`) : filtre resserré aux libellés commençant par **OP** ou **NOUVEAU** ; colonne **Prioritaire** ajoutée en placeholder (vide, pas de source de donnée pour l'instant — définition à trancher avec Andréa/Marlène quand le besoin sera précisé).
- [x] [DEC] **Fiche Achat / Article — sources de vérité à retrancher** : **validé par le métier (23/07)** — EAN/PCB → Sylob source de vérité ; le reste (marquage, matière, packaging détaillé) → fiche achat existante tant qu'elle n'est pas remplacée. Onglet Article = vue 360° (Sylob + Matrice + Fiche Achat). Onglet Fiche Achat = consultation + génération PDF + mise à jour, pas la création ex nihilo. *(Aucun code déclenché par cette validation — le périmètre actuel des onglets colle déjà à cette décision ; à vérifier lors du prochain audit écran.)*
...
- [x] Prévisionnel : vue prioritaire **par livraison/conteneur/mois** ; B/L en attente ou bloqués groupés par conteneur puis fournisseur ; vue "déjà payé" par fournisseur ET conteneur/BL -- **fait le 23/07** (`c0f432b`) : une seule vue groupée (conteneur → fournisseur) sert les 2 besoins (`/api/previsionnel.bl_par_conteneur_fournisseur`), section dédiée dans l'onglet Prévisionnel (table imbriquée conteneur/fournisseur, badge "Bloqué" + badge Paiement réutilisant `conteneurPayBadge`).
- [x] **Alertes changement ETA/livraison — implémenté le 23/07** (`docs/20260722_FUSEAU_Spec_SuiviDatesETA_v1.md`, 4 questions ouvertes arbitrées par Antho : sémantique dates confirmée, horodatage = date du fichier maritime, échelle 1/2/3+=orange/rouge/violet confirmée, ETD non historisé).
  - DDL : réutilise `achat.transport_evenement` (créée 22/07, absorbe le besoin) + nouvelle vue `achat.v_ot_transport_suivi` (nb changements cumulés + couleur, jamais remis à zéro) + colonnes `eta_maj_le`/`date_livraison_maj_le` sur `ot_transport` pour la préséance chronologique.
  - `transform_maritime.py` : propage `date_transmission` = mtime du fichier. `load_ot_gmail.py` : abandon du COALESCE sur `eta`/`date_livraison`, détection de changement + préséance chronologique (une transmission plus ancienne que la dernière déjà appliquée est ignorée), insertion d'événement.
  - Frontend : badge couleur (point orange/rouge/violet) sur les cellules ETA/Livraison de l'onglet Conteneurs + remontée dans Actions prioritaires (Dashboard).
  - ⚠️ **Bloqué en l'état** : `achat.transport_evenement` appartient à `platform_team`, dont mon login (`dtpf_sylob_anthony_bezille_prod`) n'est pas membre (écho direct du point pérennisation infra déjà tracké, section Session 20/07) → `INSERT` refusé sur `transport_evenement_id_seq` (permission denied), testé et confirmé le 23/07. Le code est correct (testé en isolant la détection de changement, ex. ETA 2026-01-19→2026-01-25 bien détectée) mais ne peut pas encore écrire en base. **Action requise** : un GRANT (ou un changement d'ownership, cohérent avec les 17 tables/6 vues/7 séquences déjà passées à `platform_team`) depuis l'identité admin Entra/`azure_pg_admin`.
  - Reste hors scope (dépendance déjà notée dans la spec, non traité le 23/07) : ingestion des changements ETA depuis le **corps des mails** (nécessite le chantier `parse_bl extract_table`) — seule la source fichier maritime alimente `transport_evenement` pour l'instant.
...
- [x] Fiche Achat — **fait le 27/07** : HO Code (optionnel, source = commentaire Sylob), label Name clarifié "(EN — Sylob)" vs "Désignation FR", vrai logo TB (asset SVG officiel Design System, remplace le bloc texte "TB" inventé). Emplacement marquage (image) + bloc AGEC intégral déjà en place depuis la reconstruction du 22/07, non retouchés. Le questionnaire docx envoyé à Andréa est resté vierge — elle a répondu par un autre canal (contenu intégral dans `docs/20260727_FicheAchat_Questionnaire_Sourcing_Andrea_COMPLETE_v1.md`, confirme aussi mot pour mot les alias fournisseurs déjà codés dans `ALIAS_CONNUS`). Ce contenu a servi à ajouter des hints (ⓘ) sur les champs à règle métier non évidente (format N° LOT AAMMJJHHMM, ETD = +90j sauf urgence, règle des échantillons à cocher selon existant/nouveau × vrac/packaging spécial, "shipping paid by" par défaut TB).
...
**Mise à jour 23/07** : 5 des 6 points 🟡 tranchés (doublons fournisseurs codé, Promo/Opé codé, Qualité/Fiche Achat actés côté process, statut Livrée bloqué sur un accès Sylob manquant plutôt qu'une décision métier). Reste ouvert : raison du retard (parsing mail) + confirmation du groupe AOYAM/HIAMEA détecté en marge de la demande initiale. Les chantiers ⬜ restent non bloquants pour le 31/07.
