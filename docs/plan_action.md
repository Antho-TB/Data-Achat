...
- [x] [DEC] **Doublons fournisseurs — tranché (23/07)** : confirmé de vrais doublons à corriger via l'ID Sylob `frn_code`, pas le nom texte — **implémenté** (`13ad399`, `enrich_ca.py`) : regroupement par composantes connexes nom↔frn_code, avec garde-fou anti-faux-positif (un lien nom↔code ambigu n'est retenu qu'avec ≥ 2 PO de corroboration — un seul PO bruité aurait sinon fusionné à tort GUANGWEI/DIAMOND TRACK avec HONGXING, 2 fournisseurs distincts). Résultat : GUANGWEI/DIAMOND TRACK et SMART IRON/JIT GLOBAL fusionnés comme demandé. **⚠️ À confirmer avec le métier** : le même algorithme détecte un 3ᵉ groupe au même niveau de preuve — **AOYAM/HIAMEA** (même frn_code 00001234, 2 et 3 PO respectivement) — pas mentionné dans la demande initiale, à valider avant la prochaine démo.
